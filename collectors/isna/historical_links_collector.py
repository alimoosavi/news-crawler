...
from .daily_links_collector import ISNADailyLinkCollector


class ISNAHistoricalLinksCollector:
    """
    Manages the historical crawl across a range of Gregorian dates (Miladi)
    for ISNA. Each day is converted to Jalali (Shamsi) before fetching.
    """
    ...
    def _crawl_single_day(g_date: dt_date) -> Tuple[str, List[NewsLinkData]]:
        """
        Crawl a single Gregorian date and fetch ISNA links for that day.
        Internally converts to Jalali for ISNADailyLinkCollector.
        """
        try:
            shamsi_date = jdatetime.date.fromgregorian(date=g_date)
            collector = ISNADailyLinkCollector(
            ...
            logging.error(f"Error crawling ISNA for {g_date}: {e}", exc_info=True)
            return str(g_date), []

    def collect_range(self, start_date: dt_date, end_date: dt_date):
        """
        Collects and persists all ISNA news links between start and end Gregorian dates (inclusive).
        Uses multiprocessing for concurrent crawling.
        """
        if start_date > end_date:
            self.logger.error("Start date cannot be after end date.")
            return

        self.logger.info(f"Starting ISNA historical crawl from {start_date} to {end_date} (Gregorian).")
        ...
        while current_date <= end_date:
            ...
            # Run the crawl in parallel
            with mp.Pool(processes=self.workers) as pool:
                results = pool.map(self._crawl_single_day, batch_dates)

            # Persist each batch immediately
            for date_str, links in results:
                if links:
                    self.db_manager.insert_new_links(links)
                self.logger.info(f"Day {date_str} completed — {len(links)} links collected.")

            current_date = batch_end + timedelta(days=1)

        self.logger.info("\n✅ ISNA historical range crawl completed.")
