"""
Async Tasnim Page Collector with Parallel Processing
...
class TasnimPageCollector:
    ...
    MAIN_CONTENT_SELECTOR = "div.story"
    ...
    def __init__(
        ...
        """
        Initialize async Tasnim collector.
        ...
        self.logger.info(
            f"TasnimPageCollector initialized: "
    ...
    async def _fetch_html_async(
        ...
        try:
            ...
            self.logger.debug(f"✓ Fetched Tasnim page: {url}")
            return html_content

        except PlaywrightTimeoutError:
            self.logger.error(f"⏱ Timeout while fetching Tasnim page: {url}")
            return None
            
        except Exception as e:
            self.logger.error(
                f"❌ Error fetching Tasnim page {url}: {e}",
                exc_info=True
            )
            return None
    ...
    async def _crawl_batch_async(
        ...
        # Filter for Tasnim links only
        tasnim_links = [link for link in batch if link.source == TASNIM]
        
        if not tasnim_links:
            return results
        
        self.logger.info(
            f"🚀 Starting async crawl of {len(tasnim_links)} Tasnim links "
        ...
        async with async_playwright() as p:
            ...
            await asyncio.gather(
                *[fetch_and_parse(link) for link in tasnim_links],
        ...
        self.logger.info(
            f"✅ Completed async crawl: {len(results)}/{len(tasnim_links)} successful"
        )
    ...
    def extract_news(
        ...
        """Extract structured news data from Tasnim HTML"""
        try:
            ...
            # Summary
            summary_tag = soup.select_one("div.lead")
            summary = summary_tag.get_text(strip=True) if summary_tag else None

            # Images
            images = []
            img_tag = soup.select_one("div.image-container img")
            if img_tag and img_tag.get("src"):
                images.append(img_tag["src"])

            # Content
            content_tag = soup.select_one("div.story-content")
            content = ""
            if content_tag:
            ...
            keywords = [
                ...
                for tag in soup.select("div.keywords a")
            ]
            ...
            self.logger.error(
                f"❌ Error parsing Tasnim HTML for {link_data.link}: {e}",
                exc_info=True
            )
            return None